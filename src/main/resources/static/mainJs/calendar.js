...
document.addEventListener('DOMContentLoaded', function () {
    var calendarEl = document.getElementById('calendar'),
        ...
        eventTitle = $('#calTitle'), //Title
        eventLabel = $('#calLabel'), //Label
        startDate = $('#calStartDate'), //Start Date
        endDate = $('#calEndDate'), //End Date
        eventUrl = $('#event-url'),
        eventGuests = $('#event-guests'),
        eventLocation = $('#calLocation'), //Location
        allDaySwitch = $('.allDay-switch'),
        selectAll = $('.select-all'),
        calEventFilter = $('.calendar-events-filter'),
        filterInput = $('.input-filter'),
        btnDeleteEvent = $('.btn-delete-event'),
        calendarEditor = $('#calNote'), //Note
        calendarCustomer = $('#calCustomer'), //Customer
        calendarDoctor = $('#calDoctor'); //Doctor
    ...
    function fetchEvents(info, successCallback) {
        ...
        $.ajax({
            url: "./calendar/listAppointment",
            type: "GET",
            contentType: "application/json; charset=utf-8",
            success:function (data){
                console.log(data)
                var calendars = selectedCalendars()
                selectedEvents = data.filter(function (event) {
                    return calendars.includes(event.calLabel.toLowerCase());
                });
            ...
            error: function (err){
                console.log(err)
                alert("Randevu Listeleme Sırasında bir hata oluştu.")
            }
        })
...
//----------------------------------------------- Add Event - Start -----------------------------------------------//
    ...
    $(addEventBtn).on('click', function () {
        if (eventForm.valid()) {
            ...
            const obj = {
                calTitle: eventTitle.val(),
                calLabel: eventLabel.val(),
                calStartDate: startDate.val(),
                calEndDate: endDate.val(),
                calCustomer: calendarCustomer.val(),
                calDoctor: calendarDoctor.val(),
                calLocation: eventLocation.val(),
                calNote: calendarEditor.val()
            }
